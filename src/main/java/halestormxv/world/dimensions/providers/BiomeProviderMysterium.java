...
import halestormxv.world.dimensions.gen.layer.GenLayerMY;
...
public class BiomeProviderMysterium extends BiomeProvider
{
    ...
    public BiomeProviderMysterium()
    {
        biomeCache = new BiomeCache(this);
        biomesToSpawnIn = new ArrayList<Biome>();
        biomesToSpawnIn.add(BiomeInit.LUPRESIUM_FOREST);
        biomesToSpawnIn.add(BiomeInit.MYSTIC_LANDS);
    }

    public BiomeProviderMysterium(long par1, WorldType par3WorldType)
    {
        this();
        GenLayer[] agenlayer = GenLayerMY.makeTheWorld(par1);
        biomeToUse = agenlayer[0];
        biomeIndexLayer = agenlayer[1];
    }

    public BiomeProviderMysterium(World par1world)
    {
    ...
    public Biome[] getBiomesForGeneration(Biome[] par1ArrayOfBiome, int par2, int par3, int par4, int par5)
    {
        ...
        for (int i = 0; i < par4 * par5; ++i)
            if (aint[i] >= 0 && aint[i] <= Biome.REGISTRY.getKeys().size())
                ...
                par1ArrayOfBiome[i] = BiomeInit.MYSTIC_LANDS;
