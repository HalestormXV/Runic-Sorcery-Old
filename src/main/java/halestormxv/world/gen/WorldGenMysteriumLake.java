...
public class WorldGenMysteriumLake extends WorldGenerator
{
    private IBlockState blockIndex;

    public WorldGenMysteriumLake(Block par1)
    {
        this(par1.getDefaultState());
    }

    public WorldGenMysteriumLake(IBlockState state)
    {
        blockIndex = state;
    }
                ...
                for (int j3 = 0; j3 < 16; ++j3)
                    for (int j4 = 4; j4 < 8; ++j4)
                        if (aboolean[(i2 * 16 + j3) * 8 + j4])
                        {
                            ...
                            if (worldIn.getBlockState(blockpos) == Blocks.STONE && worldIn.getLightFor(EnumSkyBlock.SKY, position.add(i2, j4, j3)) > 0)
                                if(n <= 32)
                                    worldIn.setBlockState(blockpos, ModFluids.LIQUID_MYSTERIUM.getBlock().getDefaultState(), 2);
                                else worldIn.setBlockState(blockpos, Blocks.STONE.getStateFromMeta(1), 2);
                        }
